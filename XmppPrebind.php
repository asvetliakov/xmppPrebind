 ...
 * PEAR Auth_SASL
 * 
 * The system uses those files stored in PEAR directories
 * The files provided in the Auth folder are there for code analysis and do not
 * 	do anything.
 * 	
 * 	-asa 
 */
...
/*		if ($this->firePhp) {
	...
	private function sendBindIfRequired() {
		if ($this->doBind) {
			$domDocument = $this->buildBody(true);
			$body = self::getBodyFromDomDocument($domDocument);
			...
			$iq->appendChild(self::getNewTextAttribute($domDocument, 'id', '_bind_auth_2'));
			...
			$iq->appendChild($bind);
			$body->appendChild($iq);
			if ($response = simplexml_load_string($this->send($domDocument->saveXML()))) {
			    $this->jid = $response->iq->bind->jid . '';
			}
			return true;
	...
	private function sendSessionIfRequired() {
		if ($this->doSession) {
			...
			$iq->appendChild(self::getNewTextAttribute($domDocument, 'xmlns', self::XMLNS_CLIENT));
			$iq->appendChild(self::getNewTextAttribute($domDocument, 'type', 'set'));
			$iq->appendChild(self::getNewTextAttribute($domDocument, 'id', '_session_auth_2' ));
	...
	private function replyToChallengeResponse($challengeResponse) {
		...
		$body->appendChild($response);
		return $domDocument->saveXML($body);
	}
	 ...
	 * @return DOMDocument
	 */
	private function buildBody($isBosh = false) {
